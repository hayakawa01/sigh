  ...
  <v-row>
    <v-col cols="4">
      <v-card class="mx-auto" max-width="344" variant="outlined">
        ...
        <v-card-actions>
          <v-btn variant="outlined" v-on:click="buttonClicked"> 送信 </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-col cols="4">
      <v-card class="mx-auto" max-width="344" variant="outlined">
        ...
        <v-card-actions>
          <v-btn variant="outlined" v-on:click="chatGptClick"> ChatGPTへ送信 </v-btn>
        </v-card-actions>
      </v-card>
      <audio class="audio"></audio>
    </v-col>
    ...
    <v-col cols="4">
      <v-card class="mx-auto" max-width="344" variant="outlined">
        <v-card-item> ずんだもんに話しかける </v-card-item>

        <v-card-actions>
          <v-btn variant="outlined" v-on:click="start"> 開始 </v-btn>
          <v-btn variant="outlined" v-on:click="end"> 終了 </v-btn>
        </v-card-actions>
      </v-card>
      <audio class="audio"></audio>
    </v-col>
...
async function getAudioURL(text) {
  ...
  const response = await axios.post('http://localhost:50021/synthesis?speaker=1', query, {
    responseType: 'blob'
  })
...
const api_key = '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'
